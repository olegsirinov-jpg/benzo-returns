...
class Notify
{
    ...
    /**
     * Нагадування клієнту: повернення погоджено, але ТТН досі немає.
     * Надсилається автоматично (крон) один раз — email + SMS/Viber.
     *
     * @param array<string,mixed> $rma
     */
    public static function remindTtn(array $rma): void
    {
        $rmaId  = (int)$rma['id'];
        $number = (string)$rma['rma_number'];
        $link   = Rma::publicUrl($rma);
        ...
        if (!empty($rma['email'])) {
            $tpl = self::pack(
                'Нагадування: завершіть повернення ' . $number,
                'Нагадуємо про ваше повернення ' . $number . '.',
                'Повернення погоджено, але ми ще не отримали від вас відправлення. '
                . 'Будь ласка, відправте товар і вкажіть номер ТТН на сторінці статусу. '
                . 'Найпростіше — безкоштовне «Легке повернення» Нової пошти: тоді номер '
                . 'вказувати не потрібно, ми побачимо його автоматично.',
                $link,
                ''
            );
            self::once($rmaId, 'ttn_reminder', 'email', (string)$rma['email'], function () use ($rma, $tpl) {
                $r = Mailer::send((string)$rma['email'], (string)($rma['customer_name'] ?? ''), $tpl['subject'], $tpl['html']);
                return [$r['ok'], $r['error']];
            });
        }

        if (!empty($rma['phone']) && TurboSms::enabled()) {
            $sms = 'Нагадуємо: щоб завершити повернення ' . $number . ', відправте товар і вкажіть '
                 . 'номер ТТН: ' . $link . ' Або оформіть безкоштовне «Легке повернення» Нової пошти.';
            self::once($rmaId, 'ttn_reminder', 'sms', (string)$rma['phone'], function () use ($rma, $sms) {
                $r = TurboSms::send((string)$rma['phone'], $sms);
                return [$r['ok'], $r['error']];
            });
        }
    }
        ...
        switch ($event) {
            ...
            case 'need_more_info':
                return self::pack(
                    ...
                    . 'Будь ласка, звʼяжіться з менеджером, щоб надати додаткові дані або фото '
                    . 'товару чи упаковки. Менеджер також може звʼязатися з вами сам.',
                    $link,
                    'Заявка ' . $number . ': потрібні додаткові дані/фото. Звʼяжіться з менеджером. ' . $link
                );
            ...
            case 'approved':
                return self::pack(
                    ...
                    ($msg !== '' ? nl2br(e($msg)) . '<br><br>' : '')
                    . 'Тепер відправте товар — способи й покрокова інструкція за посиланням нижче. '
                    . '<b>Якщо відправляєте звичайною накладною Нової пошти, після відправлення '
                    . 'обовʼязково вкажіть її номер (ТТН) на сторінці статусу</b> — інакше ми не '
                    . 'знатимемо, що очікувати посилку. Якщо оформите «Легке повернення» НП, '
                    . 'номер вказувати не потрібно — ми побачимо його автоматично.',
                    $link,
                    'Повернення ' . $number . ' погоджено. Відправте товар і вкажіть номер ТТН на сторінці '
                    . '(окрім «Легкого повернення»): ' . $link
                );
